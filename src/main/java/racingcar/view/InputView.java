...
public final class InputView {
    ...
    public static GameInfo inputGameInfo() {
        ...
        String csvCarNames = readLine();
        var carNames = commaSeparator.split(csvCarNames);
        var trimmedNonEmptyNames = Arrays.stream(carNames).map(String::trim).filter(name -> !name.isEmpty()).toList();
        return new GameInfo(trimmedNonEmptyNames);
    }

    private static String readLine() {
        return MissionUtil.instance.readLine();
    }
